...
from llama_index.node_parser.hierarchical import _add_parent_child_relationship
    ...
    def get_nodes_from_documents(
        ...
        with self.callback_manager.event(
            ...
            if self.metadata_extractor is not None:
                if self.use_async:
                    # all_nodes = run_async_tasks(
                    #     self.metadata_extractor.aprocess_nodes(all_nodes)
                    # )
                    all_nodes = asyncio.run(
                        self.metadata_extractor.aprocess_nodes(all_nodes)
                    )
                else:
...
class AsyncHierarchicalNodeParser(HierarchicalNodeParser):
    ...
    @classmethod
    def class_name(cls) -> str:
        return "AsyncHierarchicalNodeParser"
